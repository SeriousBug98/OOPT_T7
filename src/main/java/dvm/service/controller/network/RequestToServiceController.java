...
//        int otherDVMItemNum = msgContent.getItem_num();//요청받은 item num
//        int otherDVMItemCode = msgContent.getItem_code();//요청받은 item code
//
...
//        }
    ...
    public boolean sendPrepayRequest(int item_code, int item_num, String card_num, int price) {
        ...
        MsgType msgType = MsgType.req_prepay;
        MsgContent msgContent = new MsgContent(item_code,item_num,cert_code); // 아이템 코드랑 item_num은 넘겨받아야됨
//        System.out.println("RequestToServiceController.sendPrepayRequest"+msgContent);
        String src_id = "Team7";
        int selectIndex = getNearestDVMIndex(-1);
        if (selectIndex == -1){
            // 여기까지 오면 선결제 가능한 DVM이 없다는 뜻이다.
        ...
        // 위에서 isAvailability 반환 문제로 cert_code값이 제대로 출력 안돼서 일단 만들어놓음..
        returnValue[2]=cert_code;
